...
def add_invoice():
    ...
    if request.method == 'POST':
        try:
            ...
            if not village_result:
                flash(f"Village '{village_name}' not found", "alert-danger")
            ...
            for hold_type, hold_amount in zip(hold_types, hold_amounts):
                ...
                if not hold_type_result:
                    flash(f"Invalid Hold Type: {hold_type}", "alert-danger")
                    return redirect(url_for('add_invoice'))
            ...
            flash("✅ Invoice added successfully!", "alert-success")
            return redirect(url_for('add_invoice'))

        except mysql.connector.Error as e:
            connection.rollback()
            flash(f"❌ Failed to add invoice: {str(e)}", "alert-danger")
            return redirect(url_for('add_invoice'))
        finally:
            cursor.close()
            connection.close()

    # GET: show the form and invoice list
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.callproc('GetAllInvoiceDetails')
        for result in cursor.stored_results():
